...
However, for the sake of illustration, we can estimate varibable importance for nitrogen oxide only using one base learner as follows:
```{r}
## load the library
library(npvi)
...
This code takes approximately 5 seconds to run on a (not very fast) PC. Under the hood, `vim()` fits the `SuperLearner()` function with the specified library, and then returns fitted values and variable importance estimates. This is most suitable for estimating variable importance for the first feature on a given dataset. We can see these estimates:
...
This takes approximately 2 seconds --- now rather than estimating both conditional means, we are only estimating one.
...
## fit a GAM using SuperLearner, using the two-step estimating procedure
...
## Estimating variable importance for multiple variables
