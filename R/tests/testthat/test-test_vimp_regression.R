context("test-test_vimp_regression.R")
...
reduced_fit <- SuperLearner(Y = matrix(full_fitted), X = x[, -2, drop = FALSE], SL.library = learners)
...
test_that("Test ANOVA-based variable importance", {
  ...
  expect_equal(est$est, (500/729)/(1 + 2497/7875 + 500/729), tolerance = 0.01)
})
