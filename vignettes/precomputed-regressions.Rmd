...
for (v in 1:2) {
  ...
  full_mod <- glm(y ~ ., data = train_v, family = binomial())
...
for (i in seq_len(ncol(X) - geog_indx)) {
  ...
  for (v in 1:2) {
    train_v <- subset(X, vrc01_folds != v)
    test_v <- subset(X, vrc01_folds == v)
    red_fit[vrc01_folds == v] <- suppressWarnings(
      predict(glm(full_fit[vrc01_folds != v] ~ ., 
                                            data = train_v %>% 
                                              select(-!!this_name), family = binomial()),
                                         newdata = test_v)  
    )
  }
