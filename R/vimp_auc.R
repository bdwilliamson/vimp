...
#'
#' Compute estimates of and confidence intervals for nonparametric difference in $AUC$-based variable importance.
#'
#' @param Y the outcome.
...
#' @param run_regression if outcome Y and covariates X are passed to \code{vimp_auc}, and \code{run_regression} is \code{TRUE}, then Super Learner will be used; otherwise, variable importance will be computed using the inputted fitted values.
...
#'
#' est <- vimp_auc(Y = y, f1 = full.fit, f2 = red.fit,
...
vimp_auc <- function(Y, X, f1 = NULL, f2 = NULL, indx = 1, weights = rep(1, length(Y)), run_regression = TRUE, SL.library = c("SL.glmnet", "SL.xgboost", "SL.mean"), alpha = 0.05, na.rm = FALSE, f1_split = NULL, f2_split = NULL, folds = NULL, scale = "logit", pval_tol = 1e-3, ...) {
  vim(Y = Y, X = X, f1 = f1, f2 = f2, indx = indx, weights = weights, type = "auc", run_regression = run_regression, SL.library = SL.library, alpha = alpha, na.rm = na.rm, f1_split = f1_split, f2_split = f2_split, folds = folds, scale = scale, pval_tol = 1e-3, ...)
}
