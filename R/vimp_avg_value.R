...
vimp_avg_value <- function(Y, X, f1 = NULL, f2 = NULL, indx = 1, run_regression = TRUE, SL.library = c("SL.glmnet", "SL.xgboost", "SL.mean"), alpha = 0.05, na.rm = FALSE, ...) {
  ...
  ests <- onestep_based_estimator(fhat_ful, fhat_red, Y, type = "avg_value", na.rm = na.rm)
  
  ## compute the update
  update <- vimp_update(fhat_ful, fhat_red, Y, type = "avg_value", na.rm = na.rm)
  ...
  class(output) <- c("vim", "avg_value", tmp.cls)
  return(output)
}
