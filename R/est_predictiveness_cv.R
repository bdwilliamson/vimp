...
est_predictiveness_cv <- function(fitted_values, y, folds, type = "r_squared", C = rep(1, length(y)), Z = NULL, folds_Z = folds, ipc_weights = rep(1, length(C)), ipc_fit_type = "external", ipc_eif_preds = rep(1, length(C)), scale = "identity", na.rm = FALSE, ...) {
    ...
    # check whether or not we need to do ipc weighting -- if y is fully observed, i.e., is part of Z, then we don't
    ...
    if (full_type == "r_squared") {
        ...
        for (v in 1:V) {
            tmp_ics[1:length(C[folds_Z == v]), v] <- (-1) * as.vector(matrix(c(1 / var$point_est, -point_ests[v] / (var$point_est ^ 2)), nrow = 1) %*% t(cbind(ics[1:length(C[folds_Z == v]), v], var$eif[folds_Z == v])))
        }
    ...
    if (full_type == "deviance") {
        ...
        tmp_ics <- matrix(NA, nrow = max_nrow, ncol = V)
        for (v in 1:V) {
            tmp_ics[1:length(C[folds_Z == v]), v] <- (-1) * as.vector(matrix(c(1 / denom$point_est, -point_ests[v] / (denom$point_est ^ 2)), nrow = 1) %*% t(cbind(ics[1:length(C[folds_Z == v]), v], denom$eif[folds_Z == v])))
        }
