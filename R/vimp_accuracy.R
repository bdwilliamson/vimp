...
#'
#' Compute estimates of and confidence intervals for nonparametric difference in misclassification rate-based variable importance.
#'
#' @param Y the outcome.
...
vimp_accuracy <- function(Y, X, f1 = NULL, f2 = NULL, indx = 1, run_regression = TRUE, SL.library = c("SL.glmnet", "SL.xgboost", "SL.mean"), alpha = 0.05, na.rm = FALSE, 
  ...
  if (!is.null(fhat_split_ful) & !is.null(fhat_split_red)) {
    hyp_test <- vimp_hypothesis_test(fhat_split_ful, fhat_split_red, Y, folds, type = "accuracy", level = alpha, na.rm = na.rm)  
  } else {
    hyp_test <- NA
  }
  ...
  class(output) <- c("vim", "accuracy", tmp.cls)
  return(output)
}
