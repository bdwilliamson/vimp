...
Assume that our data are generated according to the mechanism $P_0$. We can then define a nonparametric measure of variable importance, \[\psi_{0, j} = \frac{\int [\mu_{P_0} - \mu_{P_0, j}]^2dP_0(x)}{\text{Var}_{P_0}(Y)},\] which is the proportion of the variability in the outcome explained by including $X_j$ in our chosen estimation technique. 

This document introduces you to the basic tools in npvi and how to apply them to a dataset. We will explore the two different ways of obtaining variable estimates using npvi:

1. You (the user) only specify a *library* of candidate estimators for the conditional means $\mu_{P_0}$ and $\mu_{P_0, j}$; you allow npvi to obtain the optimal estimates of these quantities using the `SuperLearner` [@vanderlaan2007], and use these estimates to obtain variable importance estimates
2. You (the user) have a favorite estimator for the conditional means; you simply want npvi to obtain variable importance estimates using this estimator

## Data: Boston

Throughout we will use the Boston housing study data [@harrison1978], freely available from the `MASS` package. We can use `?Boston` to see documentation for these data.
...
Now that we have created the library of learners, we can move on to estimating variable importance.
...
- `f1` and `f2`, which specify whether or not we need to estimate the conditional means 
- `data`, which supplies data to the function
- `j`, which determines the feature we want to estimate variable importance for
...
We will illustrate each of these choices in order below, but in general we use (1) to estimate variable importance for the first feature in a given dataset and (2) or (3) to estimate variable importance for subsequent features in the same dataset. Method (3) allows the most flexibility, and all time-intensive computation occurs before calling `vim()`; however, this involves more work on your (the user's) end. 

Suppose that the first feature that we want to estimate variable importance for is nitrogen oxide, `nox`. Since this is the first feature, say we choose (1) above. Then supplying `vim()` with 
...
- We want to use `SuperLearner()` to estimate the conditional mean $\mu_{P_0}$
- We want to use the two-step procedure outlined in Williamson 2017 (CITE) to estimate the conditional mean $\mu_{P_0, j}$
- We want to estimate variable importance for the fifth column of `Boston`, which is `nox`
...
However, for the sake of illustration, we can estimate varibable importance for nitrogen oxide only using only two base learners as follows:
```{r}
## load the library
library(npvi)
...
We said earlier that we want to obtain estimates of all individual features in these data, so let's choose average number of rooms (`rm`) next. Now that we have estimated variable importance for nitrogen oxide, the `full.fit` object contains our estimate of $\mu_{P_0}$. Since we have spent the time to estimate this using `SuperLearner()`, there is no reason to estimate this function again. This leads us to choose (2) above, since we have already estimated variable importance on one feature in this dataset. Using the small learners library (again only for illustration) yields
...
This takes approximately 5 seconds --- now rather than estimating both conditional means, we are only estimating one.

If we choose (3), then we have to use a single method from the library, or call `SuperLearner()` ourselves, prior to estimating variable importance. Then `vim()` returns variable importance estimates based on these fitted values. For example, let's estimate variable importance for the distance to radial highways using this approach.
...
It is important to note that if we use the same learners library, then approaches (2) and (3) are equivalent. 

We can obtain estimates for the remaining individual features in the same way (again using only two base learners for illustration):
```{r}
...
Now that we have estimates of each of individual feature's variable importance, we can view them all simultaneously by plotting:
```{r}
...
Now that we have estimated variable importance for each of the individual features, we can estimate variable importance for each of the groups that we mentioned above: accessibility features, structural features, nitrogen oxide, and neighborhood features.

The only difference between estimating variable importance for a group of features rather than an individual feature is that now we specify a vector for `j`; we can use any of the options listed in the previous section to compute these estimates.
...
plot(groups, nms.2, pch = 16, ylab = "", xlab = "Estimate", main = "Estimated variable importance for groups", xlim = c(0, 0.2), axes = FALSE)
```
