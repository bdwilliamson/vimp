#' Nonparametric Variable Importance Estimates: Difference in the average value under the optimal treatment rule
#'
#' Compute estimates of and confidence intervals for nonparametric variable importance defined as the difference in the average value under the optimal treatment rule.
#'
#' @param Y the outcome.
...
#' @param run_regression if outcome Y and covariates X are passed to \code{vimp_avg_value}, and \code{run_regression} is \code{TRUE}, then Super Learner will be used; otherwise, variable importance will be computed using the inputted fitted values.
...
#'
#' est <- vimp_avg_value(Y = y, f1 = full.fit, f2 = red.fit,
...
vimp_avg_value <- function(Y, X, f1 = NULL, f2 = NULL, indx = 1, weights = rep(1, length(Y)), run_regression = TRUE, SL.library = c("SL.glmnet", "SL.xgboost", "SL.mean"), alpha = 0.05, na.rm = FALSE, f1_split = NULL, f2_split = NULL, folds = NULL, scale = "identity", pval_tol = 1e-3, ...) {
  vim(Y = Y, X = X, f1 = f1, f2 = f2, indx = indx, weights = weights, type = "avg_value", run_regression = run_regression, SL.library = SL.library, alpha = alpha, na.rm = na.rm, f1_split = f1_split, f2_split = f2_split, folds = folds, scale = scale, pval_tol = pval_tol, ...)
}
