...
#'
#' @param Y the outcome; a list of length V.
#' @param X the covariates; a list of length V. 
...
#' @param run_regression if outcome Y and covariates X are passed to \code{vimp_regression}, and \code{run_regression = TRUE}, then Super Learner will be used; otherwise, variable importance will be computed using the inputted fitted values. 
...
cv_vim <- function(Y, X, f1, f2, indx = 1, V = 10, type = "regression", run_regression = TRUE, SL.library = c("SL.glmnet", "SL.xgboost", "SL.mean"), alpha = 0.05, na.rm = FALSE, update_denom = TRUE, ...) {
  ...
  if (run_regression) {
    stop("cv_vim() is not yet implemented without pre-computed regression estimators.")
  ...
  for (v in 1:V) {
    naive_cv[v] <- onestep_based_estimator(fhat_ful[[v]], fhat_red[[v]], Y[[v]], type = type, na.rm = na.rm)
    
    if (update_denom) { ## here, use the IC of the full standardized parameter
      updates[v] <- mean(vimp_update(fhat_ful[[v]], fhat_red[[v]], Y[[v]], type = type, na.rm = na.rm), na.rm = na.rm)
      ses[v] <- mean(vimp_update(fhat_ful[[v]], fhat_red[[v]], Y[[v]], type = type, na.rm = na.rm)^2, na.rm = na.rm) 
