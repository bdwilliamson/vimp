...
vimp_auc <- function(Y, X, f1 = NULL, f2 = NULL, indx = 1, weights = rep(1, length(Y)), run_regression = TRUE, SL.library = c("SL.glmnet", "SL.xgboost", "SL.mean"), alpha = 0.05, na.rm = FALSE,
                     f1_split = NULL, f2_split = NULL, folds = NULL, scale = "logit", ...) {
  vim(Y = Y, X = X, f1 = f1, f2 = f2, indx = indx, weights = weights, type = "auc", run_regression = run_regression, SL.library = SL.library, alpha = alpha, na.rm = na.rm,
