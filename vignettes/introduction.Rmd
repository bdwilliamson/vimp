...
Suppose that I prefer to use generalized additive models [@hastie1990] to estimate $\mu_{P_0}$ and $\mu_{P_0, s}$, so I am planning on using the `mgcv` package. Suppose that you prefer to use the elastic net [@zou2005], and are planning to use the `glmnet` package. 
...
- I want to use the two-step procedure outlined in Williamson 2017 (CITE) to estimate the conditional mean $\mu_{P_0, s}$
...
## now estimate variable importance
nox.vim <- vim(f1 = y ~ x, f2 = fit ~ x, data = Boston3, y = Boston3$medv, s = 5, SL.library = learners.2)
```
...
dis.vim <- vim(full.fit, reduced.fit, y = Boston3$medv, s = 8)
```
...
structure.vim <- vim(full.fit, fit ~ x, data = Boston3, y = Boston3$medv,
                s = c(6, 7), SL.library = learners.2)
access.vim <- vim(full.fit, fit ~ x, data = Boston3, y = Boston3$medv,
                s = c(8, 9), SL.library = learners.2)
