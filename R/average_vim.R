...
average_vim <- function(..., weights = rep(1/length(list(...)), length(list(...)))) {
    ...
    names(naives) <- "naive"
  	...
  	risk_full_se <- mean(unlist(risk_ses_full))/sqrt(sum(unlist(lapply(L, function(z) length(z$y)))))
  	risk_reduced_se <- mean(unlist(risk_ses_reduced))/sqrt(sum(unlist(lapply(L, function(z) length(z$y)))))
  	## cis
