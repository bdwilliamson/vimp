...
#'
#' Compute estimates of and confidence intervals for nonparametric deviance- or cross-entropy-based variable importance.
#'
#' @param Y the outcome.
...
#' @param run_regression if outcome Y and covariates X are passed to \code{vimp_deviance}, and \code{run_regression} is \code{TRUE}, then Super Learner will be used; otherwise, variable importance will be computed using the inputted fitted values. 
...
vimp_deviance <- function(Y, X, f1 = NULL, f2 = NULL, indx = 1, run_regression = TRUE, SL.library = c("SL.glmnet", "SL.xgboost", "SL.mean"), alpha = 0.05, na.rm = FALSE, 
  ...
  if (run_regression) {
    ...
    reduced <- SuperLearner::SuperLearner(Y = Y, X = X_minus_s, SL.library = SL.library, ...)
  ...
  ests <- onestep_based_estimator(fhat_ful, fhat_red, Y, type = "deviance", na.rm = na.rm)
  
  ## compute the update
  update <- vimp_update(fhat_ful, fhat_red, Y, type = "deviance", na.rm = na.rm)
  ...
  hyp_test <- vimp_hypothesis_test(fhat_split_ful, fhat_split_red, Y, folds, type = "deviance", level = alpha, na.rm = na.rm)
  ...
  class(output) <- c("vim", "deviance", tmp.cls)
  return(output)
}
