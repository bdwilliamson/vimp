...
full_fit_1 <- SuperLearner(Y = y[samp], X = x[samp, ], SL.library = learners, cvControl = list(V = 3))
full_fitted_1 <- predict(full_fit_1)$pred

full_fit_2 <- SuperLearner(Y = y[-samp], X = x[-samp, ], SL.library = learners, cvControl = list(V = 3))
full_fitted_2 <- predict(full_fit_2)$pred

## fit the first split; importance for X2
reduced_fit_1 <- SuperLearner(Y = full_fitted_1, X = x[samp, -2, drop = FALSE], SL.library = learners, cvControl = list(V = 3))
reduced_fitted_1 <- predict(reduced_fit_1)$pred

## fit the second split; importance for X2
reduced_fit_2 <- SuperLearner(Y = full_fitted_2, X = x[-samp, -2, drop = FALSE], SL.library = learners, cvControl = list(V = 3))
reduced_fitted_2 <- predict(reduced_fit_2)$pred

test_that("Averaging variable importance estimates works", {
  est_1 <- vim(Y = y[samp], f1 = full_fitted_1, f2 = reduced_fitted_1, run_regression = FALSE, indx = 2, type = "r_squared")
  est_2 <- vim(Y = y[-samp], f1 = full_fitted_2, f2 = reduced_fitted_2, run_regression = FALSE, indx = 2, type = "r_squared")
