...
SL.library = c("SL.xgboost", "SL.mean"))
```
...
learners.2 <- c("SL.glmnet")

## now estimate variable importance
nox.vim <- vimp_rsquared(Y = Boston$medv, X = Boston2[, -1, drop = FALSE], 
    indx = 5, run_regression = TRUE, SL.library = learners.2, V = 5, na.rm = TRUE)
```

This code takes approximately 11 seconds to run on a (not very fast) PC. Under the hood, `vimp_rsquared()` fits the `SuperLearner()` function with the specified library, and then returns fitted values and variable importance estimates. This is most suitable for estimating variable importance for the first feature on a given dataset. I can display these estimates:
...
full.fit <- do.call(c, nox.vim$full_fit)
