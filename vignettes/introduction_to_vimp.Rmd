...
The workhorse function of `vimp`, for R-squared-based variable importance, is `vimp_rsquared`. The basic arguments are
...
SL.library = c("SL.xgboost1", "SL.mean"), V = 3, env = environment())
```
...
## plug these into vim
lm.vim.crim <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.crim, indx = 1, run_regression = FALSE, type = "r_squared")
lm.vim.zn <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.zn, indx = 2, run_regression = FALSE, type = "r_squared")
lm.vim.indus <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.indus, indx = 3, run_regression = FALSE, type = "r_squared")
lm.vim.chas <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.chas, indx = 4, run_regression = FALSE, type = "r_squared")
lm.vim.nox <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.nox, indx = 5, run_regression = FALSE, type = "r_squared")
lm.vim.rm <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.rm, indx = 6, run_regression = FALSE, type = "r_squared")
lm.vim.age <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.age, indx = 7, run_regression = FALSE, type = "r_squared")
lm.vim.dis <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.dis, indx = 8, run_regression = FALSE, type = "r_squared")
lm.vim.rad <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.rad, indx = 9, run_regression = FALSE, type = "r_squared")
lm.vim.tax <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.tax, indx = 10, run_regression = FALSE, type = "r_squared")
lm.vim.ptratio <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.ptratio, indx = 11, run_regression = FALSE, type = "r_squared")
lm.vim.black <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.black, indx = 12, run_regression = FALSE, type = "r_squared")
lm.vim.lstat <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit.lstat, indx = 13, run_regression = FALSE, type = "r_squared")
...
2. Supply outcome `Y` and fitted values for cross-fit estimates of $\mu_{P_0}$ and $\mu_{P_0, s}$ with `run_regression = FALSE` and vector of folds `folds`
...
Suppose that the first feature that I want to estimate variable importance for is nitrogen oxide, `nox`. Since this is the first feature, say I choose (1) above. Then supplying `vimp_rsquared()` with 
...
vimp_rsquared(Y = Boston$medv, X = Boston2[, -1, drop = FALSE], 
...
However, for the sake of illustration, I can estimate varibable importance for nitrogen oxide only using only one base learner as follows:
```{r, message = FALSE}
## load the library
library("vimp")
...
## now estimate variable importance
nox.vim <- vim(Y = Boston$medv, X = Boston2[, -1, drop = FALSE], 
    indx = 5, run_regression = TRUE, SL.library = learners.2, na.rm = TRUE, env = environment(), type = "r_squared")
```

This code takes approximately 11 seconds to run on a (not very fast) PC. Under the hood, `vim()` with `run_regression = TRUE` fits the `SuperLearner()` function with the specified library, and then returns fitted values and variable importance estimates. This is most suitable for estimating variable importance for the first feature on a given dataset. I can display these estimates:
...
rm.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = red.fit, indx = 6, run_regression = FALSE, type = "r_squared")
...
If I choose (2), then I have to use a single method from the library, or call `SuperLearner()` myself, prior to estimating variable importance. Then `vim()` returns variable importance estimates based on these fitted values. For example, let's estimate variable importance for the distance to radial highways using this approach.
...
dis.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced.fit, indx = 8, run_regression = FALSE, type = "r_squared")
```
...
crim.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_crim,
                indx = 1, run_regression = FALSE, type = "r_squared")

reduced_zn <- predict(SuperLearner(Y = full.fit, X = Boston[, -c(2, 14)], SL.library = learners.2))$pred
zn.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_zn,
                indx = 2, run_regression = FALSE, type = "r_squared")

reduced_indus <- predict(SuperLearner(Y = full.fit, X = Boston[, -c(3, 14)], SL.library = learners.2))$pred
indus.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_indus,
                indx = 3, run_regression = FALSE, type = "r_squared")

reduced_chas <- predict(SuperLearner(Y = full.fit, X = Boston[, -c(4, 14)], SL.library = learners.2))$pred
chas.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_chas,
                indx = 4, run_regression = FALSE, type = "r_squared")

reduced_age <- predict(SuperLearner(Y = full.fit, X = Boston[, -c(7, 14)], SL.library = learners.2))$pred
age.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_age,
                indx = 7, run_regression = FALSE, type = "r_squared")

reduced_rad <- predict(SuperLearner(Y = full.fit, X = Boston[, -c(9, 14)], SL.library = learners.2))$pred
rad.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_rad,
                indx = 9, run_regression = FALSE, type = "r_squared")

reduced_tax <- predict(SuperLearner(Y = full.fit, X = Boston[, -c(10, 14)], SL.library = learners.2))$pred
tax.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_tax,
                indx = 10, run_regression = FALSE, type = "r_squared")

reduced_ptratio <- predict(SuperLearner(Y = full.fit, X = Boston[, -c(11, 14)], SL.library = learners.2))$pred
ptratio.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_ptratio,
                indx = 11, run_regression = FALSE, type = "r_squared")

reduced_black <- predict(SuperLearner(Y = full.fit, X = Boston[, -c(12, 14)], SL.library = learners.2))$pred
black.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_black,
                indx = 12, run_regression = FALSE, type = "r_squared")

reduced_lstat <- predict(SuperLearner(Y = full.fit, X = Boston[, -c(13, 14)], SL.library = learners.2))$pred
lstat.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_lstat,
                indx = 13, run_regression = FALSE, type = "r_squared")
```
...
get_nm <- function(s) {
  if (s == 1) {
    ...
    return("Avg. num. rooms")
  } else if (s == 7) {
    return("Age")
  } else if (s == 8) {
    return("Distance")
  } else if (s == 9) {
    return("Access to radial hwys")
  } else if (s == 10) {
    return("Property tax rate")
  } else if (s == 11) {
    return("Pupil-teacher ratio")
  } else if (s == 12) {
    return("Prop. black")
  } else if (s == 13) {
    return("Prop. lower status")
  }
}
...
## plot
ests$mat %>% 
  mutate(plot_labels = get_nms(ests$mat)) %>% 
  ...
  ylab("") +
  ggtitle("Variable importance estimates for individual features in the Boston housing data")
```
...
structure.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_struct, indx = c(6, 7), run_regression = FALSE, type = "r_squared")

reduced_access <- predict(SuperLearner(Y = Boston$medv, X = Boston[, -c(8, 9, 14)], SL.library = learners.2))$pred
access.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_access, indx = c(8, 9), run_regression = FALSE, type = "r_squared")

reduced_neigh <- predict(SuperLearner(Y = Boston$medv, X = Boston[, -c(1, 2, 3, 4, 10, 11, 12, 13, 14)], SL.library = learners.2))$pred
neigh.vim <- vim(Y = Boston$medv, f1 = full.fit, f2 = reduced_access,
                indx = c(1, 2, 3, 4, 10, 11, 12, 13), run_regression = FALSE, type = "r_squared")

## combine and plot
groups <- merge_vim(structure.vim, access.vim, neigh.vim, nox.vim) 
nms.2 <- c("Neighborhood", "Structure", "Accessibility", "Nitrogen oxide")
groups$mat %>% 
  mutate(plot_labels = nms.2) %>% 
  ggplot(aes(x = est, y = plot_labels)) +
  geom_point() +
  geom_errorbarh(aes(xmin = cil, xmax = ciu)) +
  xlab(expression(paste("Variable importance estimates: ", R^2, sep = ""))) +
  ylab("") +
  ggtitle("Variable importance estimates for groups of features in the Boston housing data")
```
