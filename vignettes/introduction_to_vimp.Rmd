...
The workhorse function of `vimp`, for ANOVA-based variable importance, is `vimp_anova`. The basic arguments are
...
* run_regression: a logical value telling `vimp_anova` whether or not to run a regression of Y on X
...
The code
```{r}
est_1 <- vimp_anova(Y = y, X = x, indx = 1, run_regression = TRUE, 
...
## plug these into vim
lm.vim.crim <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.crim, indx = 1, run_regression = FALSE)
lm.vim.zn <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.zn, indx = 2, run_regression = FALSE)
lm.vim.indus <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.indus, indx = 3, run_regression = FALSE)
lm.vim.chas <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.chas, indx = 4, run_regression = FALSE)
lm.vim.nox <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.nox, indx = 5, run_regression = FALSE)
lm.vim.rm <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.rm, indx = 6, run_regression = FALSE)
lm.vim.age <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.age, indx = 7, run_regression = FALSE)
lm.vim.dis <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.dis, indx = 8, run_regression = FALSE)
lm.vim.rad <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.rad, indx = 9, run_regression = FALSE)
lm.vim.tax <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.tax, indx = 10, run_regression = FALSE)
lm.vim.ptratio <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.ptratio, indx = 11, run_regression = FALSE)
lm.vim.black <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.black, indx = 12, run_regression = FALSE)
lm.vim.lstat <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit.lstat, indx = 13, run_regression = FALSE)
...
The main function for ANOVA-derived variable importance in the `vimp` package is the `vimp_anova()` function. There are five main arguments to `vimp_anova()`:
...
I will illustrate each of these choices in order below, but in practice I use (3), since it allows the most flexibility, and all time-intensive computation occurs before calling `vimp_anova()`; however, this involves more work on your end. 

Suppose that the first feature that I want to estimate variable importance for is nitrogen oxide, `nox`. Since this is the first feature, say I choose (1) above. Then supplying `vimp_anova()` with 
...
The call to `vimp_anova()` looks like this:
```{r, eval = FALSE}
## load the library
library("vimp")
...
vimp_anova(Y = Boston$medv, X = Boston2[, -1, drop = FALSE], 
...
## now estimate variable importance
nox.vim <- vimp_anova(Y = Boston$medv, X = Boston2[, -1, drop = FALSE], 
    indx = 5, run_regression = TRUE, SL.library = learners.2)
```

This code takes approximately 11 seconds to run on a (not very fast) PC. Under the hood, `vimp_anova()` fits the `SuperLearner()` function with the specified library, and then returns fitted values and variable importance estimates. This is most suitable for estimating variable importance for the first feature on a given dataset. I can display these estimates:

```{r}
nox.vim
```

The object returned by `vimp_anova()` also contains fitted values from using `SuperLearner()`; I access these using `$full_fit` and `$red_fit`. For example,
...
rm.vim <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = red.fit, indx = 6, run_regression = FALSE)
...
If I choose (2), then I have to use a single method from the library, or call `SuperLearner()` myself, prior to estimating variable importance. Then `vimp_anova()` returns variable importance estimates based on these fitted values. For example, let's estimate variable importance for the distance to radial highways using this approach.
...
dis.vim <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = reduced.fit, indx = 8, run_regression = FALSE)
```
...
structure.vim <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = reduced_struct, indx = c(6, 7), run_regression = FALSE)

reduced_access <- predict(SuperLearner(Y = Boston$medv, X = Boston[, -c(8, 9, 14)], SL.library = learners.2))$pred
access.vim <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = reduced_access, indx = c(8, 9), run_regression = FALSE)

reduced_neigh <- predict(SuperLearner(Y = Boston$medv, X = Boston[, -c(1, 2, 3, 4, 10, 11, 12, 13, 14)], SL.library = learners.2))$pred
neigh.vim <- vimp_anova(Y = Boston$medv, f1 = full.fit, f2 = reduced_access,
