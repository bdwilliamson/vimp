...
vimp_deviance <- function(Y, X, f1 = NULL, f2 = NULL, indx = 1, V = 10, weights = rep(1, length(Y)), run_regression = TRUE, SL.library = c("SL.glmnet", "SL.xgboost", "SL.mean"), alpha = 0.05, na.rm = FALSE, folds = NULL, scale = "identity", pval_tol = 1e-3, ...) {
  cv_vim(Y = Y, X = X, f1 = f1, f2 = f2, indx = indx, V = V, weights = weights, type = "deviance", run_regression = run_regression, SL.libary = SL.library, alpha = alpha, na.rm = na.rm, folds = folds, scale = scale, pval_tol = pval_tol, ...)
}
